...
from torchmdnet.module_FM_peak import LNNP
...
def get_args():
    ...
    parser.add_argument('--lr-min', type=float, default=1.0e-7, help='Minimum learning rate before early stop')
    ...
    parser.add_argument('--lr-cosine-length', type=int, default=800000, help='Cosine length if lr_schedule is cosine.')
    ...
    parser.add_argument('--splits', default='experiments/pretrain2_spec_chem_1peak_basefg_qm9sp/splits.npz', help='Npz with splits idx_train, idx_val, idx_test')
    ...
    parser.add_argument('--job-id', default="ft_pcq_1peak_qm9sp_zpve", type=str, help='Job ID')
    parser.add_argument('--pretrained-model', default="experiments/pretrain2_spec_chem_1peak_basefg_qm9sp/step=249399-epoch=289-val_loss=0.1807-test_loss=0.1809-train_per_step=0.2580.ckpt", type=str, help='Pretrained checkpoint')
    parser.add_argument('--peak-mask', type=bool, default=False, help='peak mask for spectra training')
    ...
    parser.add_argument('--dataset-arg', default="zpve", type=str, help='Target property for QM9')
    ...
    parser.add_argument('--energy-weight', default=1.0, type=float, help='Energy weight')
    ...
    parser.add_argument('--prior-model', type=str, default='Atomref', choices=priors.__all__, help='Prior model')
    ...
    parser.add_argument('--standardize', type=bool, default=True, help='Standardize')
    ...
    parser.add_argument('--contrastive-weight', default=0.0, type=float, help='Contrastive weight')

    # SpecFormer specific
    # parser.add_argument('--patch-len', type=int, nargs=3, default=[20,50,50], help='Patch lengths')
    # parser.add_argument('--stride', type=int, nargs=3, default=[20,50,50], help='Strides')
    parser.add_argument('--patch-len', type=int, nargs=3, default=[20,50,50], help='Patch lengths')
    parser.add_argument('--stride', type=int, nargs=3, default=[10,25,25], help='Strides')
    parser.add_argument('--reconstruct-weight', default=0.0, type=float, help='Reconstruct weight')
    parser.add_argument('--mask-ratios', type=float, nargs=3, default=[0.1,0.1,0.1], help='Mask ratios')
    parser.add_argument('--entire-spectrum-mask-ratio', type=float, default=0, help='entire spectrum mask ratio')
    ...
    parser.add_argument('--fusion-reconstruct-weight', default=0.0, type=float, help='Reconstruct weight')
...
def main():
    ...
    ddp_plugin = None  # 先初始化

    # 使用 PL 2.x 推荐方式
    if "ddp" in args.distributed_backend:
        ddp_plugin = "ddp"  # 直接用字符串 strategy
    ...
    trainer = pl.Trainer(
        ...
        strategy="ddp" if "ddp" in args.distributed_backend else "auto",
    ...
    # args.load_model = "/data1/biyunying/MolSpectra/experiments/pretrain_baseline/step=395599-epoch=459-val_loss=0.1360-test_loss=0.2856-train_per_step=0.1110.ckpt"
