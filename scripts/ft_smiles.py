...
from torchmdnet.module_FM_peak_smiles import LNNP
...
def get_args():
    ...
    parser.add_argument('--splits', default='experiments/pretrain2_pcq_basefg26_originqm9sp2/splits.npz', help='Npz with splits idx_train, idx_val, idx_test')
    ...
    parser.add_argument('--job-id', default="ft_smiles_gpt_lr", type=str, help='Job ID')
    parser.add_argument('--pretrained-model', default='experiments/pretrain2_pcq_basefg26_originqm9sp2/step=447199-epoch=519-val_loss=0.3947-test_loss=0.4022-train_per_step=0.3752.ckpt', type=str, help='Pretrained checkpoint')
    ...
    parser.add_argument('--smiles-weight', default=1.0, type=float, help='Smiles weight')
    ...
    parser.add_argument('--task', type=str, default="hopping", choices=["regression","smiles","hopping"], help='tasks')
...
def main():
    ...
    # print(model)
    # for param in model.model.representation_spec_model.parameters():
    #     param.requires_grad = False
    # for param in model.model.representation_spec_model.pool.parameters():
    #     param.requires_grad = True
    # for param in model.model.smiles_decoder.parameters():
    #     param.requires_grad = False

    # for name, param in model.model.smiles_decoder.named_parameters():
        
    #     # 判定解冻逻辑
    #     if "multihead_attn" in name:
    #         # Cross-Attention (连接 Encoder 的部分)
    #         param.requires_grad = True
    #     elif "linear1" in name or "linear2" in name:
    #         # FNN (Feed Forward Network)
    #         param.requires_grad = True
    #     elif "norm" in name:
    #         # LayerNorm (norm1, norm2, norm3) - 通常建议解冻
    #         param.requires_grad = True
