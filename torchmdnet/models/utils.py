...
def generate_square_subsequent_mask(sz: int, device: torch.device) -> torch.Tensor:
    """生成一个上三角的未来信息掩码 (causal mask)"""
    mask = (torch.triu(torch.ones(sz, sz, device=device)) == 1).transpose(0, 1)
    mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
    return mask

def create_masks(tgt_smiles: torch.Tensor, pad_idx: int = 0):
    """为目标SMILES序列创建所有需要的掩码"""
    # 填充掩码: 标记出所有PAD token的位置
    tgt_key_padding_mask = (tgt_smiles == pad_idx) # [bs, seq_len]
    
    # 未来信息掩码: 防止模型在预测第i个token时看到第i+1个及之后的token
    tgt_mask = generate_square_subsequent_mask(tgt_smiles.size(1), tgt_smiles.device) # [seq_len, seq_len]
